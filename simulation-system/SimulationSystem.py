...
class SimulationSystem(abc.ABC):
    ...
    def add_component(self, num_of_inputs, outputs, length, tick_length, radius, type):
        ...
        if type == "pipe":
            component = SplitterPipe.SplitterPipe(
                id_num, num_of_inputs, outputs, length, tick_length, radius)
        if type == "filter":
            component = SandFilter.SandFilter(
                id_num, num_of_inputs, outputs, length, tick_length, radius)
        if type == "sink":
            component = Sink.Sink(id_num, num_of_inputs,
                                  outputs, length, tick_length, radius)
        if type == "source":
            component = Source.Source(
                id_num, outputs, length, tick_length, radius)
