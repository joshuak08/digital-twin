...
class SplitterPipe(GenericPipe):
    ...
    def push(self, flow_in, time):
        ...
        # currently DOES NOT DO TIME PROPERLY, we need all components to keep track of the oldest time (perhaps by pointer)

        self.time = max(time, self.time) + 1  # increases time by 1 after a push
        self.capacity = self.capacity + flow_in - self.outputRate*len(self.outputs)  # increase capacity difference of input and output
        ...
        for child_pipe in self.outputs:       # iterates through all output pipes
            self.time = max(self.time, child_pipe.push(self.outputRate, self.time))  # updates current time with oldest time and pushes flow
    # ================================== #
